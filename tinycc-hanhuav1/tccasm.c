...
static void asm_parse_directive(TCCState *s1, int global)
{
	...
	case TOK_ASMDIR_对齐:
    case TOK_ASMDIR_b对齐:
    case TOK_ASMDIR_p2对齐:
    case TOK_ASMDIR_跳过:
    case TOK_ASMDIR_空间:
        tok1 = tok;
        next();
        n = asm_int_expr(s1);
        if (tok1 == TOK_ASMDIR_p2align || tok1 == TOK_ASMDIR_p2对齐)
        {
            if (n < 0 || n > 30)
                tcc_error("invalid p2align, must be between 0 and 30");
            n = 1 << n;
            tok1 = TOK_ASMDIR_align || TOK_ASMDIR_对齐;
        }
        if (tok1 == TOK_ASMDIR_align || tok1 == TOK_ASMDIR_balign || tok1 == TOK_ASMDIR_对齐 || tok1 == TOK_ASMDIR_b对齐) {
            if (n < 0 || (n & (n-1)) != 0)
    ...
    case TOK_ASMDIR_区段:
	    ...
	    if (tok1 == TOK_ASMDIR_section || tok1 == TOK_ASMDIR_区段)
	        use_section(s1, sname);
	    else
...
ST_FUNC void asm_instr(void)
{
    ...
    /* since we always generate the asm() instruction, we can ignore
       volatile */
    if (tok == TOK_VOLATILE1 || tok == TOK_VOLATILE2 || tok == TOK_VOLATILE3 || tok == TOK_VOLATILE1_CN || tok == TOK_VOLATILE2_CN || tok == TOK_VOLATILE3_CN) {
        next();
    }
    parse_asm_str(&astr);
    nb_operands = 0;
