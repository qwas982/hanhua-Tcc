 ...
 *  TCC - 极小 C 编译器
 * 
 *  版权所有（c）2001-2004  法布里斯·贝拉德
 *
 * 这个代码库是自由软件; 你可以重新分配它 和/或
 * 根据由自由软件基金会公布的许可证 GNU LGPL 中的条款进行修改; 任选其一
 * 第2版的许可证，或（由你选择）任何后续的版本。
 *
 * 该代码库是分布式的期望它是有用的，
 * 但没有任何保证; 甚至没有可销售的隐含保证
 * 或适用于特定目的。 参见GNU
 * LGPL获得更多细节。
 *
 * 您应该已经收到了该代码库的GNU LGPL 通用公共许可证的副本; 如果没有，请写信给自由软件基金会，公司.，
 * 地址；寺庙广场59号，第330号套房 ，波士顿，马萨诸塞 02111-1307 美国
 */
...
redo:
    if (tok != TOK_ATTRIBUTE1 && tok != TOK_ATTRIBUTE2 && tok != TOK_ATTRIBUTE1_CN && tok != TOK_ATTRIBUTE2_CN)
        return;
    next();
    skip('(');
    skip('(');
    while (tok != ')') {
        ...
        case TOK_SECTION1:
        case TOK_SECTION2:
		case TOK_SECTION1_CN:
		case TOK_SECTION2_CN:
            skip('(');
	...
	case TOK_VISIBILITY1_CN:
	case TOK_VISIBILITY2_CN:
            skip('(');
	    parse_mult_str(&astr,
        ...
        case TOK_ALIGNED1:
        case TOK_ALIGNED2:
		case TOK_ALIGNED1_CN:
		case TOK_ALIGNED2_CN:
            if (tok == '(') {
                next();
                n = expr_const();
                if (n <= 0 || (n & (n - 1)) != 0) 
        ...
        case TOK_NORETURN1:
        case TOK_NORETURN2:
		case TOK_NORETURN1_CN:
		case TOK_NORETURN2_CN:
            /* currently, no need to handle it because tcc does not
               track unused objects */
            break;
        case TOK_CDECL1:
        ...
        case TOK_STDCALL2:
        case TOK_STDCALL3:
		case TOK_STDCALL1_CN:
		case TOK_STDCALL2_CN:
		case TOK_STDCALL3_CN:
            ad->f.func_call = FUNC_STDCALL;
            break;
#ifdef TCC_TARGET_I386
        case TOK_REGPARM1:
        case TOK_REGPARM2:
		case TOK_REGPARM1_CN:
		case TOK_REGPARM2_CN:
            skip('(');
            n = expr_const();
            if (n > 3) 
                n = 3;
        ...
        case TOK_FASTCALL2:
        case TOK_FASTCALL3:
		case TOK_FASTCALL1_CN:
		case TOK_FASTCALL2_CN:
		case TOK_FASTCALL3_CN:
		...
		case TOK_MODE_CN:
            ...
            break;
        case TOK_DLLEXPORT:
		case TOK_DLLEXPORT_CN:
            ad->a.dllexport = 1;
            break;
        case TOK_DLLIMPORT:
		case TOK_DLLIMPORT_CN:
            ad->a.dllimport = 1;
            break;
        default:
...
static int parse_btype(CType *type, AttributeDef *ad)
{
    ...
    while(1) {
		...
		case TOK_VOLATILE2_CN:
		case TOK_VOLATILE3_CN:
            type->t = t;
		...
		case TOK_RESTRICT2_CN:
		case TOK_RESTRICT3_CN:
            next();
            break;
        case TOK_UNSIGNED:
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
	...
	case TOK_builtin_expect_CN:
	/* __builtin_expect is a no-op for now */
	parse_builtin_params(0, "ee");
	vpop();
        break;
    case TOK_builtin_types_compatible_p:
	case TOK_builtin_types_compatible_p_CN:
	...
	case TOK_builtin_choose_expr_CN:
	{
	    int64_t c;
	    next();
	    skip('(');
	...
	case TOK_builtin_constant_p_CN:
	...
	case TOK_builtin_frame_address_CN:
    case TOK_builtin_return_address:
	case TOK_builtin_return_address_CN:
        {
            int tok1 = tok;
            int level;
            next();
            ...
            if (tok != TOK_CINT) {
                tcc_error("%s only takes positive integers",
                          tok1 == (TOK_builtin_return_address || TOK_builtin_return_address_CN) ?
            ...
            if (tok1 == TOK_builtin_return_address || tok1 == TOK_builtin_return_address_CN) {
    ...
    case TOK_builtin_va_arg_types_CN:
...
#ifdef TCC_TARGET_ARM64
    case TOK___va_start || TOK___va_start_CN: {
	parse_builtin_params(0, "ee");
        //xx check types
        gen_va_start();
        vpushi(0);
    ...
    case TOK___va_arg || TOK___va_arg_CN: {
    ...
    case TOK___arm64_clear_cache || TOK___arm64_clear_cache_CN: {
	parse_builtin_params(0, "ee");
        gen_clear_cache();
        vpushi(0);
        vtop->type.t = VT_VOID;
...
static void block(int *bsym, int *csym, int is_expr)
{
    ...
    if (tok == TOK_GOTO || tok == TOK_GOTO_CN) {
        ...
        skip(';');
    } else if (tok == TOK_ASM1 || tok == TOK_ASM2 || tok == TOK_ASM3 || tok == TOK_ASM1_CN || tok == TOK_ASM2_CN || tok == TOK_ASM3_CN) {
        asm_instr();
    } else {
        b = is_label();
        if (b) {
...
static int decl0(int l, int is_for_loop_init, Sym *func_sym)
{
    ...
    while (1) {
        if (!parse_btype(&btype, &ad)) {
            ...
            if (l == VT_CONST &&
                (tok == TOK_ASM1 || tok == TOK_ASM2 || tok == TOK_ASM3 || tok == TOK_ASM1_CN || tok == TOK_ASM2_CN || tok == TOK_ASM3_CN)) {
                /* global asm block */
                asm_global_instr();
                continue;
            }
		...
		type.ref = sym_push(SYM_FIELD, &type.ref->type, 0, type.ref->c);
            ...
            if (gnu_ext && (tok == TOK_ASM1 || tok == TOK_ASM2 || tok == TOK_ASM3 || tok == TOK_ASM1_CN || tok == TOK_ASM2_CN || tok == TOK_ASM3_CN)) {
