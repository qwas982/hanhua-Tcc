...
{
    next_nomacro();
    if (tok == TOK_push_macro || tok == TOK_pop_macro || tok == TOK_push_macro_CN || tok == TOK_pop_macro_CN) {
        int t = tok, v;
        Sym *s;
        ...
        if (next(), tok != ')')
            goto pragma_err;
        if (t == TOK_push_macro || t == TOK_push_macro_CN) {
    ...
    } else if (tok == TOK_once || tok == TOK_once_CN) {
        ...
        next();
        skip('(');
        if (tok == TOK_ASM_pop || tok == TOK_ASM_pop_CN) {
            next();
            ...
            int val = 0;
            if (tok != ')') {
                if (tok == TOK_ASM_push || tok == TOK_ASM_push_CN) {
                    next();
    ...
    } else if (tok == TOK_comment || tok == TOK_comment_CN) {
        char *p; int t;
        next();
        skip('(');
        t = tok;
        ...
        if (tok != ')')
            goto pragma_err;
        if (t == TOK_lib || t == TOK_lib_CN) {
            dynarray_add(&s1->pragma_libs, &s1->nb_pragma_libs, p);
        } else {
            if (t == TOK_option || t == TOK_option_CN)
...
static void parse_escape_string(CString *outstr, const uint8_t *buf, int is_long)
{
    ...
    for(;;) {
        ...
        if (c == '\\') {
            ...
            /* assume we are processing UTF-8 sequence 假设我们正在处理UTF-8序列 */
            /* reference: The Unicode Standard, Version 10.0, ch3.9 参考：Unicode标准，版本10.0，ch3.9 */

            int cont; /* count of continuation bytes 连续字节数 */
            int skip; /* how many bytes should skip when error occured 错误发生时应跳过多少字节 */
            int i;

            /* decode leading byte 解码前导字节 */
            if (c < 0xC2) {
            ...
            /* decode continuation bytes 解码连续字节 */
            for (i = 1; i <= cont; i++) {
                int l = 0x80, h = 0xBF;

                /* adjust limit for second byte 调整第二个字节的限制 */
                if (i == 1) {
                    switch (c) {
    ...
    Sym *s)
{
    ...
    /* if symbol is a macro, prepare substitution 如果符号是宏，准备替换 */
    /* special macros 特殊宏 */
...
static void pp_debug_defines(TCCState *s1)
{
    ...
    if (t == TOK_DEFINE || t == TOK_DEFINE_CN) {
        ...
        fprintf(fp, "#undef %s\n", vs);
    } else if (t == TOK_push_macro || t == TOK_push_macro_CN) {
        fprintf(fp, "#pragma push_macro(\"%s\")\n", vs);
    } else if (t == TOK_pop_macro || t == TOK_pop_macro_CN) {
